...
Vec3f ImportanceSampleGGX(Vec2f Xi, Vec3f N, float roughness) {
    ...
    float cosTheta = std::sqrtf((1.0f - Xi.y) / (1.0f + (a * a - 1.0f) * Xi.y));
    float sinTheta = std::sqrtf(1.0f - cosTheta * cosTheta);

    //TODO: from spherical space to cartesian space - Bonus 1
    Vec3f H;
    H.x = std::cosf(phi) * sinTheta;
    H.y = std::sinf(phi) * sinTheta;
    H.z = cosTheta;

    //TODO: tangent coordinates - Bonus 1
    Vec3f right = Vec3f(1.0f, 0.0f, 0.0f);
    Vec3f tangent = normalize(cross(right, N));
...
Vec3f IntegrateBRDF(Vec3f V, float roughness) {
    ...
    for (int i = 0; i < sample_count; i++) {
        ...
        if (NoL > 0.0f)
        {
            ...
            float Fc = std::powf(1.0f - VoH, 5.0f);
